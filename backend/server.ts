...
try {
  ...
  // Debug log environment variables (except sensitive ones)
  console.log('Environment Variables:');
  console.log('PORT:', process.env.PORT);
  console.log('NODE_ENV:', process.env.NODE_ENV);
  console.log('ATXP_CONNECTION_STRING:', process.env.ATXP_CONNECTION_STRING ? '*** (set)' : 'not set');
} catch (error) {
...
console.log('ATXP_CONNECTION_STRING:', ATXP_CONNECTION_STRING ? '*** (set)' : 'not set');

if (!ATXP_CONNECTION_STRING) {
  throw new Error('ATXP_CONNECTION_STRING is not set');
}

// Debug: Log the connection string components
try {
  const url = new URL(ATXP_CONNECTION_STRING);
  const params = new URLSearchParams(url.search);
  const token = params.get('connection_token');
  console.log('Connection URL:', url.origin);
  console.log('Connection token present:', !!token);
  // Don't log the actual token for security
} catch (e) {
  console.warn('Error parsing connection string:', e);
}
...
try {
  ...
  // Log the original connection string for debugging
  console.log('Original connection string:', ATXP_CONNECTION_STRING);
  
  // Create the account without modifying the connection string
  account = new ATXPAccount(ATXP_CONNECTION_STRING, {
    network: 'base',
    // Add any additional options that might affect address handling
  });
  
  console.log('ATXPAccount initialized successfully');
} catch (error) {
  console.error('Error initializing ATXPAccount:', error);
  throw error;
}
...
app.post('/api/texts', async (req: Request, res: Response) => {
  ...
  // The ATXP server will handle the address case sensitivity
